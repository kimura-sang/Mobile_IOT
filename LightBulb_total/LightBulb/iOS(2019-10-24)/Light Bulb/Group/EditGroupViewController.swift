...
class EditGroupViewController: UIViewController {
    ...
    @IBAction func clickDelete(_ sender: Any) {
        UtilFunctions.showConfirmDialog("North Bulb", "You want to delete this group?", "Yes", "No", self, {
        ...
        if __DEVICES.count > 0 {
            for i in 0...(__DEVICES.count - 1) {
                ...
                if __GROUPS.count > 0 && groupId > -1 {
                    selectedFlag = __GROUPS[groupId].deviceUUIDs.contains(__DEVICES[i].deviceUUID) ? true : false
                }
    ...
    func saveGroup(_ selectedDeviceNames: String, _ selectedDevices: [String]) {
        ...
        if groupId == -1 {
            let groupModel = GroupModel.init(id: getLastGroupId(), image: groupImageName, name: groupName!, deviceNames: selectedDeviceNames, uuids: selectedDevices, status: false)
            
            __GROUPS.append(groupModel)
        } else {
            __GROUPS[groupId].groupName = edtGroupName.text!
            __GROUPS[groupId].groupImage = groupImageName
            __GROUPS[groupId].deviceUUIDs = selectedDevices
